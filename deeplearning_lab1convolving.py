...
X_training = Xtr[:1000] / 255  # Normalize training data
Y_training = Ytr[:1000]
X_test = Xte / 255  # Normalize test data
...
complete_NN = ConvNet([
    padding(),
    ConvLayer(filters=16, kernel_size=(3, 3)),
    Pooling(pool_size=(2, 2)),
    relu(),
    padding(),
    ConvLayer(filters=32, kernel_size=(3, 3)),
    Pooling(pool_size=(2, 2)),
    relu(),
    ConvLayer(filters=64, kernel_size=(3, 3)),
    relu(),
    reshaping(),
...
for i in range(epochs):
    ...
    for j in range(batches, 1001, batches):
        ...
        print(f"Epoch: {i+1} \t Batch: {j//batches} \t Loss: {loss:.4f} \t Accuracy: {accur:.2f}%")
        if accur >= stopper:
            broke = 1
            break
        ...
        k = j
    if broke:
        print("Stopping early as accuracy reached stopper value.")
        break

out = complete_NN.forward_pass(X_training)
final_loss = CE.loss(out, Y_training)
final_accuracy = acc.value(out, Y_training) * 100
print(f"The final loss on training set: {final_loss:.4f}")
print(f"The final accuracy on training set: {final_accuracy:.2f}%")

# Evaluate on test set
out_test = complete_NN.forward_pass(X_test)
test_accuracy = acc.value(out_test, Yte) * 100
print(f"The accuracy on test set: {test_accuracy:.2f}%")